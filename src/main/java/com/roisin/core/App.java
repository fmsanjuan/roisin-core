...
public class App {
	...
	public static void main(String[] args) {
		...
		try {
			...
			System.out.println("\nCon bucle\n");
			// Variable inicial con el nœmero registros de los datos de ejemplo.
			int numCasos = 0;
			// Predicciones posibles
			List<String> labelNames = ruleModel.getLabel().getMapping().getValues();
			// C‡lculo del nœmero de registros de los datos de ejemplo a partir
			...
			for (Rule rule : ruleModel.getRules()) {
				...
				System.out.println("La precisi—n de la regla nœmero " + ruleNum + " es de "
						+ ruleConfidences[labelIndex]);
				int[] ruleFrequencies = rule.getFrequencies();
				System.out.println("El soporte de la regla nœmero " + ruleNum + " es de "
